...
def trans(samples):
    samples = np.delete(samples, np.s_[3:6], axis=0)
    array_ret= np.zeros(66)
    array_ret[:63] = samples
    return array_ret
...
def load_gestures(dataset='dhg', root='./dataset_dhg1428', version_x='3D', version_y='both',
    ...
    for idx in range(gestures.shape[0]):
        for idx2 in range(gestures[idx].shape[0]):
            gestures[idx][idx2]= trans(gestures[idx][idx2])
    gestures = gestures[:, :, :-3]
